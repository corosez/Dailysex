    ...
    def upload_file_to_dropbox(self, file_path: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                direct_download_link = shared_link_metadata.url.replace('?dl=0', '?dl=1')
            except dropbox.exceptions.ApiError as e:
                if e.error.is_shared_link_already_exists():
                    links = dbx.sharing_list_shared_links(path=dropbox_path).links
                    if links:
                        direct_download_link = links[0].url.replace('?dl=0', '?dl=1')
                    else:
                        raise Exception("Could not retrieve existing shared link.")
                else:
                    raise
